...
class MessagesManager {
    ...
    func getMessagesWithAsyncAwait(page: Int) async throws -> [Message] {
        return try await withCheckedThrowingContinuation { continuation in
            let query = GraphQLMessagesQuery(page: page)
            apolloClient.fetch(query: query) { result in
                ...
                case .success(let response):
                    ...
                    continuation.resume(returning: messagesGraphQL.map { Message(graphQLMessageEntity: $0) } )
        ...
        apolloClient.fetch(query: query) { result in
            ...
            case .success(let response):
                if let messagesGraphQL = response.data  {
                    subject.send(messagesGraphQL.map { Message(graphQLMessageEntity: $0) })
                } else {
